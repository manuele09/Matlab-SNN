...
%prima seq: ABBA
%seconda seq: BDDC (più rewarding)
%test: A, B
...
for k = 1:size(matrici, 1)
    ...
    if riga(2) == 16     %Context Layer
        ...
        v = riga(5); %voltage
        u = riga(6); %u
        corr = riga(8); %currente
        spk = riga(7); %NSpikes
        idp=riga(11); %id parent (father)
        idt=riga(12); %id target
        
        %bisogna investigare quando entrambi sono -1.
        if idp==-1 && idt==-1
...
%in pratica toglie l'ultima dimensione
%vengono fissate le prime 2 dimensioni e si fa la media al variare della
%terza
...
%figure
subplot(rows, columns, index_plot);
...
stringa = sprintf("Context Layer, Sim: %d", indice);
