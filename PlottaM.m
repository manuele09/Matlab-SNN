function PlottaM (indice,sind) 
%Plotta MorriLecar
%Con questa funzione vengono plottati solo i neuroni simulati nelle
%simNumberInternal, quindi in uscita avremo 3 grafici:
%
% - Potenziali di membrana del Morris Lecar
% - Grafico con target ed errore rispetto al target
% - Potenziali dei neuroni sommatori
...
dati = importdata(['../Dati/Neurons',num2str(indice),'-',num2str(sind),'.txt']);
dati2 = importdata(['../Dati/NeuronMorrisLecar',num2str(indice),'-',num2str(sind),'.txt']);
end
matrici = dati.data;
matrici2= dati2.data;
clear dati;
...
%   
 ...
 w=1;
 for k = 1: size(potenziali15_ML, 2)
         membrane15_ML = [];
         membrane15=[];
         corr15_ML=[];
            for i=1:numberSteps
            membrane15_ML(i) = potenziali15_ML(2,k,i);
            membrane15(i) = potenziali15(1,k,i);
            corr15_ML(i) = correnti15_ML(2,k,i);
            end     
       subplot(size(potenziali15_ML, 2),1,w);
       w=w+1;
        plot(membrane15_ML, 'b');
        hold on
        plot(corr15_ML,'r');
        plot(membrane15,'k');
        hold off;
...
%  %% Plot sameness neuron - NO
%  figure
%  subtitle('Sameness Neuron')
%   w=1;
%  for k = 1: size(potenziali17, 2)
%          membrane15 = [];
%             for i=1:numberSteps
%             membrane15(i) = potenziali17(3,k,i);
%             end     
%        subplot(size(potenziali17, 2),1,w);
%        w=w+1;
%         plot(membrane15, 'b');
...
%  
       ...
       %% Plot Target with error
 figure
 set(gcf,'numbertitle','off','name',['Current_with_target_sim',num2str(indice+1)]) % See the help for GCF 
 if sind~=1
    target = importdata(['../Dati/target', num2str(indice),'.txt']);
 else
    target = importdata(['../Dati/target', num2str(indice),'-',num2str(sind),'.txt']);
 end
 plot(target(:,1), 'b')
 hold on
 plot(target(:,2), 'r')
